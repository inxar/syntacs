 ...
 */
public class StandardParser extends AbstractLRTranslationComponent implements Parser {
  ...
  public void initialize(Object arg) {
    if (arg instanceof DPA) this.dpa = (DPA) arg;
    else throw new IllegalArgumentException("Require DPA initialization argument.");
  }
  ...
  public void notify(Symbol symbol) throws TranslationException {
      ...
      switch (cx.getType()) {
        ...
        case Correction.WAIT:
          ...
          if (symbol.getSymbolType() == waitSymbol) {
            if (DEBUG && debug)
              log().debug().write("WAIT satisfied, moving to next correction instruction.").out();
        ...
        case Correction.TUMBLE:
          ...
          while (!states.isEmpty()) {
            ...
            if (dpa.action(state, symbol.getSymbolType()).getType() != Action.ERROR) {

              if (DEBUG && debug)
                log()
                    .debug()
                    .write("TUMBLED into state ")
                    .write(state)
                    .write(" ")
            ...
            if (stack.isEmpty()) throw new TranslationException(auditor, "Tumbled Out of Parse");
    ...
    switch (action.getType()) {
      case Action.SHIFT:
        ...
        if (DEBUG && debug) log().debug().write("Shifted state ").write(state).out();
      ...
      case Action.ERROR:
        if (DEBUG && debug) log().debug().write("SYNTAX ERROR!").out();

        rx = interpreter.recover(symbol.getSymbolType(), stack);

        if (rx != null)
          if (rx.hasNext()) cx = rx.next();
          else rx = null;

        // Make error message
        if (errlim > 0 && auditor.errors() > errlim) throw new TranslationException(auditor);
        else
          auditor.notify(
  ...
  private String getSymbolName(int ID) {
    ...
    if (name == null) name = grammar.getNonTerminal(ID);
    return name;
  }

  private Log log() {
    if (log == null) log = Mission.control().log("prs", this);
    return log;
  }
  ...
  private static final class ParseStack implements Sentence {
