 ...
 */
public class StandardLexer extends AbstractLRTranslationComponent implements Lexer {
  ...
  public void initialize(Object arg) {
    if (arg instanceof DFA) this.dfas = new DFA[] {(DFA) arg};
      ...
      if (dfas.length > 1) this.stack = new ArrayIntStack();
    } else
      throw new IllegalArgumentException(
          "Initialization argument must be a single " + "DFA or an array of DFA, not " + arg);
  }

  public void start() throws TranslationException {
    this.m_context = grammar.getStartContext();
    if (dfas.length > 1) stack.push(m_context);
  ...
  public void resume() throws TranslationException {
    ...
    if (this.dfas == null)
      throw new IllegalStateException("The Lexer must be initialized before it can be run.");
    ...
    while (true) {
      ...
      try {
        ...
        if (DEBUG && debug) {
          ...
          if (c < 0) debug.write("EOF");
          else debug.write('\'').write((char) c).write('\'');
          ...
          switch (t) {
            ...
            case Token.ERROR:
              ...
              break local;
            default:
              debug.write(grammar.getTerminal(t)).write(" starting at ").write(me);
              if (nt > 0) debug.write(" and ending at ").write(nt - 1);
          }
          ...
          if (c > 0) {
            ...
            debug
                .write("DFA says: \"move from state ")
                .write(p)
                .write(" to ")
                .write(q)
                .write(" over ")
        ...
        if (p == DFA.DEAD_STATE) {
          ...
          switch (t) {
            case Token.UNDEF:
              if (DEBUG && debug)
                log().debug().write("Moving from pluripotent to open-error").out();
            ...
            case Token.ERROR:
              if (DEBUG && debug)
                if (mt < 0) log().debug().write("Moving from open-error to open-error").out();
                else log().debug().write("Moving from phoenix-error to open-error").out();
            ...
            default:
              if (DEBUG && debug)
                log().debug().write("Moving from open-token to closed-token").out();
              ...
              if (DEBUG && debug)
                log().debug().write("Backing up ").write(off - nt).write(" chars").out();
              ...
              // break out;
              if (m_isPaused) break main;

              if (DEBUG && debug)
                log()
                    .debug()
                    .write("Moving from closed-token to pluripotent" + " (through tabla-rasa)")
                    .out();
                ...
                switch (grammar.getContextAction(cp, t)) {
                  ...
                  default:
                    throw new UnsupportedOperationException(
                        "Unsupported Context Action: " + grammar.getContextAction(cp, t));
                }
              }
          ...
          if (DEBUG && debug)
            log().debug().write("Output for state ").write(p).write(" is ").write(u).out();
          ...
          if (u != Token.UNDEF) {
            ...
            if (t == Token.ERROR) {
              ...
              if (DEBUG && debug)
                if (mt < 0) log().debug().write("Moving from open-error to open-token").out();
                else log().debug().write("Moving from phoenix-error to open-token").out();
            ...
            if (DEBUG && debug) {
              if (t == Token.UNDEF)
                log().debug().write("Moving from pluripotent to open-token").out();
              else log().debug().write("Moving from open-token to open-token").out();
            }
            ...
            if (DEBUG && debug)
              log().debug().write("Moving from open-error to pheonix-error").out();
        ...
        switch (t) {
          ...
          case Token.ERROR:
            if (DEBUG && debug)
              log()
                  .debug()
                  .write("Moving from pluripotent to closed-error " + "(through open-error by EOF)")
                  .out();
          ...
          default:
            if (DEBUG && debug) log().debug().write("Moving from open-token to closed-token").out();
        ...
        if (DEBUG && debug) log().debug().write("Notifying STOP").out();
  ...
  private Log log() {
    if (log == null) log = Mission.control().log("lex", this);
    return log;
  }
