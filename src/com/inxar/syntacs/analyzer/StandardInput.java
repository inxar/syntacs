...
public class StandardInput implements Input {
  ...
  public void initch(Object src) throws IOException {
    if (src instanceof char[]) init((char[]) src);
    else if (src instanceof URL) init((URL) src);
    else if (src instanceof Reader) init((Reader) src);
    else if (src instanceof File) init((File) src);
    else if (src instanceof String) init((String) src);
    else if (src == null)
      throw new IllegalArgumentException("A non-null initialization argument is required");
    else
      throw new IllegalArgumentException(
          "I can only be initialized using a char[], URL, File, "
              + "or Reader instance, not "
              + src);
  }
  ...
  public void bach(int length) {
    while (length-- > 0) if (LF == src[--currentPosition]) currentLine--;
  }

  public int getch() throws IOException, EOFException {
    if (DEBUG) log().debug().write("getch(): now at ").write(currentPosition).out();
  ...
  public char retch(int offset) {
    if (offset < 0) throw new ArrayIndexOutOfBoundsException("Negative array offset: " + offset);
  ...
  private Log log() {
    if (this.log == null) this.log = Mission.control().log("inp", this);
    return log;
  }
