...
public class ArrayDPA implements DPA {
   ...
   */
  public ArrayDPA(int[][] action, int[][] go, Action[] actions) {
    this.action = action;
    this.go = go;
  ...
  public Action action(int state, int symbol) {
    try {
      ...
      if (DEBUG)
        log()
            .debug()
            .write("ArrayDPA.action(): action at state ")
            .write(state)
            .write(", symbol ")
            .write(symbol)
            .write(", action is ")
            ...
            .write(", action.length ")
            .write(action.length)
            .write(", action[state].length ")
            .write(action[state].length)
            .out();

      return action[state].length <= symbol || action[state][symbol] == 0
          ? ERROR
      ...
      if (state < 0)
        throw new ArrayIndexOutOfBoundsException("Bad state index (negative?): " + state);
      else if (state >= action.length)
        throw new ArrayIndexOutOfBoundsException(
            "State " + state + " is not valid.  The highest numbered state is " + action.length);
      else if (symbol < 0)
        throw new ArrayIndexOutOfBoundsException("Bad symbol index (negative?): " + symbol);
      else if (symbol >= action[state].length)
        throw new ArrayIndexOutOfBoundsException(
            "Symbol "
                + (char) symbol
                + " is not valid for state "
                + state
                + ".  The highest numbered traversal is "
                + action[state].length);
      else throw aioobex;
    }
  }
  ...
  private Log log() {
    if (log == null) log = Mission.control().log("apa", this); // Array Pushdown Automaton
    return log;
  }
