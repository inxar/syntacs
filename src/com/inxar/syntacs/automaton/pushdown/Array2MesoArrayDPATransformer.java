 ...
 */
public class Array2MesoArrayDPATransformer implements DPATransformer {
  ...
  public DPA transform(Object array_dpa) {
    ...
    // done
    return new MesoArrayDPA(new_action, new_go, new_actions);
  }
  ...
  private int[][] compress(int[][] input) {
    ...
    while (last >= 0) if (input[--last] != null) break;
    ...
    for (int i = 0; i < len; i++) output[i] = compress(input[i]);
  ...
  private int[] compress(Action[] input, int[][] action_table) {
    ...
    if (DEBUG)
      log().debug().write("compressing ").write(action_table.length).write(" states.").out();
    ...
    for (int i = 0; i < action_table.length; i++) {
      ...
      if (state == null) {
        if (WARN) log().warn().write("compress(): state ").write(i).write(" is null.").out();
  ...
  private Log log() {
    if (this.log == null) this.log = Mission.control().log("a2m", this); // Array To Meso
    return this.log;
  }
