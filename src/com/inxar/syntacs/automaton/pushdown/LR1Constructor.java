 ...
 */
public class LR1Constructor extends LRConstructor implements Vizualizable {
  ...
  protected void init() throws AmbiguityException {
    ...
    if (DEBUG) log().debug().write("init(): Start production is ").write(start).out();

    // * get it's first item
    Item initial = start.getInitialItem();

    if (DEBUG) log().debug().write("init(): Initial start item is ").write(initial).out();
    ...
    if (DEBUG) log().debug().write("init(): initialSet").write(initialSet).out();

    // we want to set this as the start state
    State startState = lookup(initialSet);

    if (DEBUG) log().debug().write("init(): assigning start state ").write(startState).out();
    ...
    while (!stack.isEmpty()) {
      ...
      while (currentItems.hasNext()) {
        ...
        if (DEBUG) log().debug().write("currently processing item ").write(currentItem).out();
        ...
        if (nextSymbol != null) {
          ...
          if (nextSymbol.isTerminal()) {
            ...
            if (((Terminal) nextSymbol).getToken().getID() == Token.STOP) {
              dpa.accept(currentState.getID(), nextSymbol.getID());
            } else {
              dpa.shift(
                  currentState.getID(), nextSymbol.getID(), currentState.go(nextSymbol).getID());
            }
          }
  ...
  public IntSet closure(IntSet set) {
    ...
    while (!stack.isEmpty()) {
      ...
      if (DEBUG)
        log().debug().write("closure(): processing item ").write(ID).write(": ").write(item).out();
      ...
      if (item.hasNext() && (!item.nextSymbol().isTerminal())) {
        ...
        if (first.isEmpty()) {
          ...
          for (int i = 0; i < alternatives.length(); i++) {
            ...
            if ((!closure.contains(ID)) && (!stack.contains(ID))) stack.push(ID);
          }
          ...
          for (int i = 0; i < alternatives.length(); i++) {
            ...
            while (firsts.hasNext()) {
              ...
              // 'a' == item.getLookahead().
              ID = core.lookahead(terminal.isNullable() ? item.getLookahead() : terminal).getID();
              ...
              if (DEBUG)
                log()
                    ...
                    .write((terminal.isNullable() ? item.getLookahead() : terminal))
                    .out();
    ...
    if (DEBUG)
      log()
          .debug()
          .write("closure(): final closed set is:")
          .writeln()
  ...
  public IntSet getNextSet(IntSet items) {
    ...
    while (iter.hasNext()) {
      ...
      if (item.nextSymbol() != null) set.put(item.nextSymbol().getID());
    }
    // give it up...
    return set;
  }
  ...
  protected IntSet go(IntSet set, int grammarSymbolID) {
    ...
    if (set.isEmpty()) return EmptyIntSet.EMPTY_SET;
    ...
    while (items.hasNext()) {
      ...
      if (item.hasNext()) {
        if (item.nextSymbol().getID() == grammarSymbolID) {
          ...
          // this one.
          gotoSet.put(item.nextItem().lookahead(item.getLookahead()).getID());
        }
      }
    }
    if (DEBUG)
  ...
  private String toItems(IntSet items) {
    ...
    while (i.hasNext()) b.append(grammar.getItem(i.next())).append(StringTools.NEWLINE);
    return b.toString();
  }
  ...
  public void vizualize(GraphViz dot) {
    ...
    if (Mission.control().isNotTrue("viz-dpa-hide-loopback-edges"))
      log()
          .warn()
          .write(
              "Warning! Only GraphViz visualization of shift "
                  + "transitions is currently available for LR1")
          .out();
  }

  private Log log() {
    if (log == null) log = Mission.control().log("cpc", this); // Canonical Pushdown Constructor
    return log;
  }
