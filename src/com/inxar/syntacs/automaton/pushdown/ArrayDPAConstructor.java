 ...
 */
public class ArrayDPAConstructor implements DPAConstructor, Vizualizable {
  ...
  public void shift(int state, int symbol, int nextState) throws AmbiguityException {
    Action action = getAction(state, symbol);
    if (action.getType() == Action.ERROR) allocAction(state, symbol, Action.SHIFT, nextState);
    else if (action.getType() != Action.SHIFT && action.getValue() != nextState)
      conflict(state, symbol, Action.SHIFT, nextState, action);
  }

  public void reduce(int state, int symbol, int production) throws AmbiguityException {
    Action action = getAction(state, symbol);
    if (action.getType() == Action.ERROR) allocAction(state, symbol, Action.REDUCE, production);
    else if (action.getType() != Action.REDUCE && action.getValue() != production)
      conflict(state, symbol, Action.REDUCE, production, action);
  }

  public void accept(int state, int symbol) throws AmbiguityException {
    Action action = getAction(state, symbol);
    if (action.getType() == Action.ERROR) allocAction(state, symbol, Action.ACCEPT, 0);
    else if (action.getType() != Action.ACCEPT) conflict(state, symbol, Action.ACCEPT, 0, action);
  }

  protected void conflict(int state, int symbol, int type, int register, Action action) {
    if (DEBUG)
      log().debug().write("conflict: ").write(action.getType()).write(", ").write(type).out();
    ...
    if (WARNING) log.warn().write(b.toString()).out();
  }
   ...
   * not.
   */
  public void go(int state, int symbol, int nextState) throws AmbiguityException {
  ...
  protected Action[] getActions() {
    ...
    // iterate them
    for (int i = 0; i < actions.size(); i++) _actions[i] = (Action) actions.elementAt(i);
  ...
  protected String show(IntSet set) {
    ...
    while (iter.hasNext()) buf.append(grammar.getItem(iter.next())).append(StringTools.NEWLINE);
    return buf.toString();
  }
  ...
  protected void showActionTable(StringBuffer buf) {
    buf.append(StringTools.NEWLINE).append("Parse table is as follows").append(StringTools.NEWLINE);
    ...
    for (int i = 0; i < action.length; i++) {
      ...
      for (int j = 0; j < action[i].length; j++) {
        a = getAction(i, j);
        if (a == null || a.getType() == Action.ERROR) continue;
  ...
  protected void showGotoTable(StringBuffer buf) {
    buf.append(StringTools.NEWLINE).append("Goto table is as follows").append(StringTools.NEWLINE);
  ...
  public void vizualize(GraphViz dot) {
    ...
    rankdir = Mission.control().getString("viz-dpa-rankdir", "LR");
    nc = Mission.control().getString("viz-dpa-node-color", "black");
    ns = Mission.control().getString("viz-dpa-node-shape", "circle");

    size = Mission.control().getString("viz-dpa-size");
    if (size != null) dot.attr("size", size);

    if (Mission.control().isTrue("viz-dpa-concentrate-edges")) dot.attr("concentrate", "true");
    ...
    if (Mission.control().isNotTrue("viz-dpa-hide-terminal-edges")) {
      ...
      ec = Mission.control().getString("viz-dpa-terminal-edge-color", "steelblue1");
      es = Mission.control().getString("viz-dpa-terminal-edge-style", "solid");
      ...
      outer:
      for (int i = 0; i < action.length; i++) {
        if (action[i] == null) continue outer;

        inner:
        for (int j = 0; j < action[i].length; j++) {
          if (action[i][j] == 0) continue inner;
          ...
          switch (a.getType()) {
            case Action.SHIFT:
              ...
              if (label == null) label = g.getNonTerminal(j);
    ...
    if (Mission.control().isNotTrue("viz-dpa-hide-nonterminal-edges")) {
      ...
      ec = Mission.control().getString("viz-dpa-nonterminal-edge-color", "royalblue");

      es = Mission.control().getString("viz-dpa-nonterminal-edge-style", "solid");
      ...
      outer:
      for (int i = 0; i < go.length; i++) {
        if (go[i] == null) continue outer;
        inner:
        for (int j = 0; j < go[i].length; j++) {
          if (go[i][j] == 0) continue inner;
  ...
  private Log log() {
    if (log == null) log = Mission.control().log("apc", this); // Array Pushdown Constructor
    return log;
  }
  ...
  protected static int[][] ensureSize(int dim1, int dim2, int[][] aa) {
    if (dim1 >= aa.length) aa = grow2(dim1, aa);

    if (aa[dim1] == null) aa[dim1] = new int[dim2 + 1];

    if (dim2 >= aa[dim1].length) aa[dim1] = grow(dim2, aa[dim1]);
