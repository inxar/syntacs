    ...
    implements Algorithm
{
	...
	if (set != null) {
		...
		if (DEBUG) trace("marked x "+x+" at min (x,y) ("+map.get(y)+","+map.get(y)+") to "+Math.min(map.get(x), map.get(y)));

		// merge the functions
		if (DEBUG) trace("unioning "+x+":"+output.get(x)+" with "+y+":"+output.get(y));
	...
	if (DEBUG) trace("stack is "+stack+", x is "+x+", depth is "+depth);
        ...
        if (map.get(x) == depth) {
	    if (DEBUG) trace("stack is "+stack+", x is "+x+", map x is "+map.get(x)+", depth is "+depth);
    ...
    private IntSet copy(IntSet set)
    {
		...
		("Please use Cloneable set implementations with this algorithm.");
        }
    }
    ...
    public static class NonTrivialSCCException
