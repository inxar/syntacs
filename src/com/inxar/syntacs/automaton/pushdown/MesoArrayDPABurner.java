...
public class MesoArrayDPABurner implements Burner {
  ...
  public void burn(Object src, ClassDeclaration cls) {
    if (DEBUG) log().debug().write("src is ").write(src).out();

    if (!(src instanceof MesoArrayDPA))
      throw new ClassCastException(
          "I can only work instances of "
              + MesoArrayDPA.class.getName()
              + ", not `"
    ...
    if (Mission.control().isFalse("compile-pickle")
        || Mission.control().isFalse("compile-dpa-pickle")) initStaticInitializerNotPickle(cls, si);
  ...
  protected void initStaticInitializerPickle(ClassDeclaration cls, StaticInitializer si) {
    ...
    invoke =
        vm.newInvoke("Pickler", "unpickle2D").addArg(BurnTools.split(Pickler.pickle(dpa.action)));
    si.newStmt(vm.newAssign(vm.newVar("_action"), invoke));

    invoke = vm.newInvoke("Pickler", "unpickle2D").addArg(BurnTools.split(Pickler.pickle(dpa.go)));
    si.newStmt(vm.newAssign(vm.newVar("_go"), invoke));

    invoke =
        vm.newInvoke("Pickler", "unpickle1D").addArg(BurnTools.split(Pickler.pickle(dpa.actions)));
  ...
  private Log log() {
    if (this.log == null) this.log = Mission.control().log("mpb", this); // Meso Pushdown Burner
    return log;
  }
