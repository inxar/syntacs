 ...
 */
public class LALR1Constructor implements DPAConstructor, Vizualizable {
  ...
  public DPA construct(ContextFreeSet grammar) throws AlgorithmException {
    ...
    if (verbose) log().info().write("Constructing LR(0) Machine (shifts)").time();

    // make the LR(0) Machine
    this.automaton = this.new LR0Automaton();
    this.automaton.init();

    if (verbose) log().info().write("Constructing lookahead sets (reductions)").time();
    ...
    if (verbose) log().info().write("Finalizing DPA construction").time();
  ...
  private NTTransition newNTTransition(State state, NonTerminal label) {
    // make a new ntt
    NTTransition nt_transition = new NTTransition(nt_transitions.size(), state, label);
    // add to the vector
  ...
  private TTransition newTTransition(State state, Terminal label) {
    // make a new ntt
    TTransition t_transition = new TTransition(t_transitions.size(), state, label);
    // add to the vector
  ...
  public void vizualize(GraphViz dot) {
    ...
    if (Mission.control().isTrue("viz-dpa-hide-loopback-edges")) return;

    lookahead.vizualize(dot);
  }

  private Log log() {
    if (log == null) log = Mission.control().log("lpc", this); // Lalr Pushdown Constructor
    return log;
  }
  ...
  final class State {
    ...
    private void classify(Item item) {
      ...
      if (!item.hasPrevious()) {
        if (DEBUG) log().debug().write("--> item is initial: ").write(item).out();
      ...
      if (!item.hasNext()) {
        if (DEBUG) log().debug().write("--> item is final: ").write(item).out();
        ...
        if (isMarked(symbol.getID())) return;
        ...
        if (symbol.isTerminal()) {
          ...
          if (DEBUG)
            log().debug().write("--> item transitions over a terminal: ").write(item).out();

          // Make a terminal transition.
          t_links = new TTransitionLink(newTTransition(this, (Terminal) symbol), t_links);

        } else {

          if (DEBUG)
            log().debug().write("--> item transitions over a nonterminal: ").write(item).out();

          // Make a nonterminal transition.
          nt_links = new NTTransitionLink(newNTTransition(this, (NonTerminal) symbol), nt_links);
    ...
    private boolean isNullable(Item item) {
      ...
      while (item.hasNext()) {
        ...
        if (!item.nextSymbol().isNullable()) return false;
        // advance
        item = item.nextItem();
      }
      // all were nullable
    ...
    private void closure(NonTerminal nonTerminal) {
      if (DEBUG) log().debug().write("--> evaluating closure over ").write(nonTerminal).out();
  ...
  final class Lookahead implements Vizualizable {
    ...
    void read() throws LRkViolationException {
      ...
      for (int i = 0; i < nt_transitions.size(); i++) {
        ...
        next = automaton.lookup(nt_transition.state.go.get(nt_transition.label.getID()));
    ...
    private void reads(IntRelation reads) throws LRkViolationException {
      ...
      SCCTransitiveClosure graph = new SCCTransitiveClosure(nt_set, reads, input, output);
    ...
    private void rewind(Item item, State state, int qB) {
      ...
      if (item.hasPrevious()) {
        ...
        // in lambda
        IntIterator previous = state.og.get(item.previousSymbol().getID()).iterator();

        // iterate all possible previous states
        while (previous.hasNext())
          // wind back line to the previous state
          rewind(item.previousItem(), automaton.lookup(previous.next()), qB);
        ...
        if (pA == -1) throw new InternalError("Huh?");
    ...
    void lookahead() throws LRkViolationException {
      ...
      */
      ...
      // make a new graph algorithm object on these arguments
      SCCTransitiveClosure graph = new SCCTransitiveClosure(nt_set, includes, input, output);
    ...
    public void vizualize(GraphViz dot) {
      LRTranslatorGrammar g = (LRTranslatorGrammar) Mission.control().get("_lr-translator-grammar");
      ...
      ec = Mission.control().getString("viz-dpa-loopback-edge-color", "midnightblue");

      es = Mission.control().getString("viz-dpa-loopback-edge-style", "dotted");
  ...
  private static class LRkViolationException extends AlgorithmException {
  ...
  private static class TTransition extends Transition {
  ...
  private static class NTTransition extends Transition {
  ...
  private static class DirectReadSetFunction extends VectorSetFunction {
  ...
  private static class ReadSetFunction extends VectorSetFunction {
  ...
  private static class FollowSetFunction extends VectorSetFunction {
  ...
  private void printNonTerminalRelation(String heading, IntRelation relation, StringBuffer b) {
    heading(heading, b);
  ...
  private void printReductionRelation(String heading, IntRelation relation, StringBuffer b) {
    heading(heading, b);
    IntIterator i = relation.keys().iterator();
    while (i.hasNext()) {
      int key = i.next();
