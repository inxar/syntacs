 ...
 * parse tables.
 */
public class MesoArrayDPA implements DPA, java.io.Serializable {
   ...
   */
  public MesoArrayDPA(int[][] action, int[][] go, int[] actions) {
    this.action = action;
    this.go = go;
  ...
  public Action action(int state_num, int symbol) {
    if (DEBUG)
      log().debug().write("MesoArrayDPA: action table length is ").write(action.length).out();
    ...
    if (DEBUG)
      log()
          ...
          .write(state_num)
          .write(" is ")
          .write(state == null ? "null" : "non-null")
          .out();
    ...
    if (symbol < state[LOWER] || symbol > state[UPPER]) {
      ...
      if (DEBUG) log().debug().write("raw encoded action is ").write(action).out();
  ...
  private Log log() {
    if (log == null) log = Mission.control().log("mpa", this); // Meso Pushdown Automata
    return log;
  }
