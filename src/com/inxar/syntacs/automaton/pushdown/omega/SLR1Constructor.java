 ...
 */
public class SLR1Constructor extends LRConstructor implements Vizualizable {
  ...
  protected void init() throws AmbiguityException {
    ...
    if (DEBUG) log().debug().write("assigning start state ").write(startState).out();
    ...
    while (!stack.isEmpty()) {
      ...
      while (nexts.hasNext()) {
        ...
        if (!seen.contains(nextSet)) stack.push(nextSet);
      }
      ...
      while (currentItems.hasNext()) {
        ...
        if (nextSymbol != null) {
          ...
          followSet = getFollowSet(currentProduction.getNonTerminal());
          ...
          while (follows.hasNext()) {
            dpa.reduce(currentState.getID(), follows.next(), currentProduction.getID());
          }
        }
      }
    }
  ...
  protected IntSet go(IntSet set, int grammarSymbolID) {
    ...
    while (items.hasNext()) {
      ...
      // test symbol
      if (item.hasNext())
        if (item.nextSymbol().getID() == grammarSymbolID) gotoSet.put(item.nextItem().getID());
    }
  ...
  protected IntSet closure(IntSet set) {
    ...
    boolean[] nonTerminalsAdded = new boolean[grammar.terminals() + grammar.nonTerminals() + 1];
        ...
        if (!symbol.isTerminal()) {
          ...
          if (!nonTerminalsAdded[nonTerminal.getID()]) {
            ...
            for (int i = 0; i < alternatives.length(); i++) {
              ...
              stack.push(grammar.getProduction(alternatives.at(i)).getInitialItem().getID());
            }
      ...
      log()
          ...
          .write(
              "Warning! Only GraphViz visualization of shift "
                  + "transitions is currently available for SLR1")
          .out();
  }
  ...
  private Log log() {
    if (log == null) log = Mission.control().log("spc", this); // Simple Pushdown Constructor
    return log;
  }
