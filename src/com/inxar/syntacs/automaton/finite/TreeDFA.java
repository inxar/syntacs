 ...
 * Lexer.
 */
public class TreeDFA implements DFA, Vizualizable {
  ...
  public int go(int state, int input) {
    ...
    while (true) {
      if (edge == null) {
        ...
        if (DEBUG) log().debug().write("got null").out();
  ...
  protected void tokens_toBuffer(StringBuffer b) {
    ...
    for (int i = 0; i < table.length; i++) {
      // see if its boring
      if (table[i].output == Token.UNDEF) continue;
  ...
  public void vizualize(GraphViz dot) {
    ...
    rankdir = Mission.control().getString("viz-dfa-rankdir", "LR");
    nc = Mission.control().getString("viz-dfa-node-color", "black");
    ns = Mission.control().getString("viz-dfa-node-shape", "circle");
    ec = Mission.control().getString("viz-dfa-edge-color", "steelblue1");
    es = Mission.control().getString("viz-dfa-edge-style", "solid");

    size = Mission.control().getString("viz-dfa-size");
    if (size != null) dot.attr("size", size);

    if (Mission.control().isTrue("viz-dfa-concentrate-edges")) dot.attr("concentrate", "true");
    ...
    for (int i = 1; i < table.length; i++) table[i].vizualize(dot);
  }

  private Log log() {
    if (log == null) log = Mission.control().log("tfa", this); // Tree Finite Automata
    return log;
  }
  ...
  public static class Edge {
    ...
    public void toBuffer(StringBuffer b) {
      ...
      if (lo == hi) b.append("[ (" + lo + ") -> " + next + " ]").append(StringTools.NEWLINE);
      else b.append("[ (" + lo + "," + hi + ") -> " + next + " ]").append(StringTools.NEWLINE);
    }
    ...
    public void vizualize(GraphViz dot) {
      if (lo < 0) {
        ...
        String lec = Mission.control().getString("viz-dfa-label-edge-color", "black");
        String les = Mission.control().getString("viz-dfa-label-edge-style", "dotted");
      ...
      if (lo == hi) edge.attr("label", str(lo));
      else edge.attr("label", "[" + str(lo) + "-" + str(hi) + "]");
    }
    ...
    private String str(int val) {
      switch (val) {
        ...
        default:
          return val > 0x20 && val <= 0xFF ? String.valueOf((char) val) : String.valueOf(val);
      }
    }
  ...
  public static Edge balance(Edge tree) {
    ...
    // iterate vector
    for (int i = 0; i < v.size(); i++) edges[i] = (Edge) v.elementAt(i);
