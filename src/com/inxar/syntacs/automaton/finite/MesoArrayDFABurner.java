...
public class MesoArrayDFABurner implements Burner {
  ...
  public void burn(Object src, ClassDeclaration cls) {
    if (!(src instanceof MesoArrayDFA))
      throw new ClassCastException(
          "I can only work instances of "
              + MesoArrayDFA.class.getName()
              + ", not `"
    ...
    con.newStmt(
        vm.newInvoke(null, "super").addArg(vm.newVar("_table")).addArg(vm.newVar("_accepts")));
    ...
    fld1.setAccess(Access.PRIVATE);
    fld1.isStatic(true);
    fld1.isFinal(true);
    fld2.setAccess(Access.PRIVATE);
    fld2.isStatic(true);
    fld2.isFinal(true);
    ...
    if (Mission.control().isFalse("compile-pickle")
        || Mission.control().isFalse("compile-dfa-pickle")) initStaticInitializerNotPickle(cls, si);
    else initStaticInitializerPickle(cls, si);
  }

  protected void initStaticInitializerNotPickle(ClassDeclaration cls, StaticInitializer si) {
  ...
  protected void initStaticInitializerPickle(ClassDeclaration cls, StaticInitializer si) {
    cls.getUnit().addImport("com.inxar.syntacs.util.Pickler");

    Invoke invoke = null;

    invoke =
        vm.newInvoke("Pickler", "unpickle2D").addArg(BurnTools.split(Pickler.pickle(dfa.table)));
    si.newStmt(vm.newAssign(vm.newVar("_table"), invoke));

    invoke =
        vm.newInvoke("Pickler", "unpickle1D").addArg(BurnTools.split(Pickler.pickle(dfa.accepts)));
  ...
  private Log log() {
    if (this.log == null) this.log = Mission.control().log("mfb", this); // Meso Finite Burner
    return log;
  }
