 ...
 * makes things faster.
 */
public class TreeDFAConstructor implements DFAConstructor {
  ...
  private void init() throws CloneNotSupportedException {
    ...
    while (!stack.isEmpty()) resolve((State) stack.pop());
  }

  private State lookup(IntSet interval_set) {
    if (DEBUG) log().debug().write("lookup(): invoked with set ").write(interval_set).out();
  ...
  private void resolve(State state) throws CloneNotSupportedException {
    if (DEBUG)
      log()
          .debug()
          .write("resolve(): -------- " + "building interval tree for state ")
          .write(state.id)
    ...
    if (DEBUG) log().debug().write("resolve(): bubble tree looks like ").write(tree).out();
  ...
  private TreeDFA.Edge traverse(BubbleTree.Bubble bubble, State state)
    ...
    // make a new edge
    return new TreeDFA.Edge( // edge has same lo/hi as this bubble node
        bubble.lo,
        bubble.hi,
        state.id,
  ...
  public DFA construct(RegularSet regset) {
    if (DEBUG) log().debug().write("construct():  regset is :").write(regset).out();
  ...
  private Log log() {
    if (log == null) log = Mission.control().log("tfc", this); // Tree Finite Constructor
    return log;
  }
