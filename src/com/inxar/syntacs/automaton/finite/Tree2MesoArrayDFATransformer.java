 ...
 */
public class Tree2MesoArrayDFATransformer implements DFATransformer {
  ...
  private int[] transform(TreeDFA.Edge tree) {
    ...
    if (tree == null || (tree.lo == MIN && tree.hi == MAX) || (tree.lo < MIN && tree.hi < MIN))
      return DEAD;
    ...
    while (true) {
      if (front.left == null || front.left.lo < MIN) break;
      else front = front.left;
    }
    while (true) {
      if (back.right == null || back.right.hi > MAX) break;
      else back = back.right;
    }
