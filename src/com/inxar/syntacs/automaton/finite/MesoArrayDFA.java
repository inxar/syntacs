...
public class MesoArrayDFA implements DFA, Serializable {
  ...
  protected void tokens_toBuffer(StringBuffer b) {
    ...
    for (int i = 0; i < accepts.length; i++) {
      // see if its boring
      if (accepts[i] == Token.UNDEF) continue;
  ...
  private void writeState(int state) {
    ...
    StringBuffer v = new StringBuffer(), l = new StringBuffer(), c = new StringBuffer();
    ...
    for (int idx = 0; idx < table[state].length; idx++) {
      ...
      // write the line
      l.append('+');
      for (int i = Math.max(ilen, vlen); i > 0; i--) l.append('-');

      // write the index
      c.append('|');
      for (int i = ipad; i > 0; i--) c.append(' ');
      c.append(idx);

      // write the value
      v.append('|');
      for (int i = vpad; i > 0; i--) v.append(' ');
      v.append(val);
    }
    l.append('+');
    c.append('|');
