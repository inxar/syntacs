 ...
 */
public class RECharClass implements CharClass {
  ...
  public String toString() {
    ...
    // middle matter
    int len = v.size();
    for (int i = 0; i < len; i++) buf.append(v.elementAt(i));
  ...
  private void init() {
    ...
    for (int i = 0; i <= Character.MAX_VALUE; i++) {
      ...
      // isNot are different
      if (chars[i] ^ isNot) intervals.addElement(grammar.newInterval(lo, hi));
      else continue;
    }
    ...
    if (len == 0) {
      throw new RuntimeException("Character classes must specify at least one character!");
    } else if (len == 1) {
      regex = (RegularExpression) intervals.elementAt(0);
    } else {
      Union union = grammar.newUnion();
      for (int i = 0; i < len; i++) union.addAllele((RegularExpression) intervals.elementAt(i));
      regex = union;
    }
