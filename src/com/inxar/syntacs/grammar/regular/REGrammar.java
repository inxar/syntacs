 ...
 */
public class REGrammar implements RegularSet, RegularGrammar {
  ...
  public synchronized RegularToken newToken(int tokenID, String tokenName, RegularExpression re) {
    if (DEBUG)
      log().debug().write("newToken(): adding token ").write(tokenID).write(": ").write(re).out();
  ...
  public synchronized RegularToken newToken(int tokenID, String tokenName, String re) {
    ...
    try {
      ...
      regex = ((Regexp) rtrans.translate(new StringReader(re))).toRegularExpression(this);

    } catch (TranslationException tex) {
      throw new RuntimeTranslationException(
          tex.getAuditor(), "Could not parse regular expression.");
    }
  ...
  public IntSet getStart() {
    if (first == null) {
      ...
      if (DEBUG) log().debug().write("getStart(): first set is ").write(first).out();
    }
  ...
  public String toString() {
    StringBuffer b =
        new StringBuffer("INTERVALS (interval followset)")
    ...
    for (int i = 0; i < intervals.size(); i++) {
      ...
      b.append(interval).append(": ").append(interval.getFollowSet()).append(StringTools.NEWLINE);
    }
    ...
    for (int i = 0; i < tokens.size(); i++) {
      ...
      b.append(token).append(": ").append(token.getRegularExpression()).append(StringTools.NEWLINE);
    }

    return b.append("-------------------").append(StringTools.NEWLINE).toString();
  }

  private Log log() {
    if (log == null) log = Mission.control().log("rg" + name.charAt(0), this);
    return log;
  }
   ...
   */
  private static final class REToken implements RegularToken {
