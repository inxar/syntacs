 ...
 */
public class CFLR1Item implements LR1Item {
  /**
  ...
  public LR1Item lookahead(Terminal lookahead) {
    if (this.lookahead == lookahead) return this;
    else return core.lookahead(lookahead);
  }
  ...
  public String toString() {
    if (string == null) {
      ...
      StringBuffer b =
          new StringBuffer("[").append(core.production.getNonTerminal()).append(" -> ");

      IntArray array = core.production.getGrammarSymbols();
      int len = array.length();

      for (int i = 0; i <= len; i++) {
        if (i == core.position) b.append("<>");
        if (i < len) b.append(core.grammar.getGrammarSymbol(array.at(i)));
      }
