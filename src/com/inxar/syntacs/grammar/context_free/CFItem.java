 ...
 */
public class CFItem implements Item {
  /**
  ...
  public LR1Item lookahead(Terminal lookahead) {
    ...
    if (lr1s == null) lr1s = new java.util.Hashtable();
  ...
  public IntSet getFirstSet() {
    if (first == null) {
      ...
      while (item.hasNext()) {
        ...
        else item = item.nextItem(); /* advance */
      }
    }
    return first;
  }
  ...
  public String toString() {
    if (string == null) {
      ...
      StringBuffer b = new StringBuffer("[ ").append(production.getNonTerminal()).append(" -> ");

      IntArray array = production.getGrammarSymbols();
      int len = array.length();
      for (int i = 0; i <= len; i++) {
        if (i > 0) b.append(' ');
        if (i == position) b.append("<> ");
        if (i < len) b.append(grammar.getGrammarSymbol(array.at(i)));
      }
