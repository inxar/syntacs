 ...
 */
public class CFGrammar implements ContextFreeGrammar, ContextFreeSet {
  ...
  public Terminal newTerminal(Token type) {
    ...
    if (type == null)
      throw new NullPointerException("newTerminal(Token) requires a non-null argument.");
  ...
  public NonTerminal newNonTerminal(String name) {
    ...
    if (name == null)
      throw new NullPointerException("newNonTerminal(String) requires a non-null String argument.");
  ...
  private void compileTerminals() {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (index >= grammarSymbols.length) grammarSymbols = grow(grammarSymbols, index);
      // now make sure that the element has not already been assigned
      else if (grammarSymbols[index] != null)
        throw new RuntimeException(
            "There is a tokenID conflict between " + grammarSymbols[index] + " and " + terminal);
  ...
  protected final void print(String name, Object[] a) {
    ...
    if (DEBUG) log().debug().write(b).out();
  }
  ...
  private Log log() {
    if (log == null) log = Mission.control().log("cfg", this);
    return log;
  }
