 ...
 */
public class CFProduction implements Production {
  ...
  public String toString() {
    ...
    buf.append('<').append(ID).append(": ").append(nonTerminal).append(" ->");
  ...
  private static class GrammarSymbolList implements IntArray {
    ...
    public int at(int index) {
      if (index < 0 || index >= count)
        throw new ArrayIndexOutOfBoundsException(
            "Request for index " + index + ", but effective size of array is " + count);

      if (src[index] == null)
        throw new NullPointerException("Request for index " + index + " has a null entry.");
    ...
    public int[] toArray() {
      ...
      for (int i = 0; i < count; i++) dst[i] = src[i].getID();
      return dst;
    }

    void add(GrammarSymbol s) {
      if (count == src.length) enlarge();
      src[count++] = s;
    }
