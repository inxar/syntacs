...
public class Buffer implements Cloneable {
  ...
  public Buffer set(int offset, char[] that_src, int that_off, int length) {
    ...
    while (this.src.length < offset + length) realloc();
  ...
  public Buffer add(String val, int offset, int length) {
    if (val == null) val = String.valueOf(val);

    while (this.src.length < this.len + length) realloc();

    if (DEBUG)
      System.out.println(
          "Copying from string with length "
              + val.length()
              + " starting at "
              + offset
              + " and ending at "
              + (offset + length)
              + " into array with length "
              + this.src.length
              + " starting at "
              + this.len);
  ...
  public Buffer add(Buffer that) {
    while (this.src.length < this.len + that.len) realloc();
  ...
  public Buffer add(char[] that_src, int that_off, int that_len) {
    while (this.len + that_len > this.src.length) realloc();
  ...
  public Buffer setCharAt(int i, char c) {
    while (this.src.length < i) realloc();
  ...
  public void length(int that_len) {
    while (this.src.length < that_len) realloc();
  ...
  public void realloc() {
    if (DEBUG) System.out.println("realloc to " + (src.length * 2));
