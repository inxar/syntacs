...
public class DOM {
   ...
   */
  public static Document getDocument(String uri, boolean validate) throws ClassNotFoundException {
    Document doc = null;
    ...
    try {
      ...
      // builder class..
      Method newInstance = dbfClass.getMethod("newInstance", null);

      Method setValidating = dbfClass.getMethod("setValidating", new Class[] {boolean.class});

      Method newDocumentBuilder = dbfClass.getMethod("newDocumentBuilder", null);

      Method parse = dbClass.getMethod("parse", new Class[] {String.class});
    ...
    try {
      ...
      Method parse = c.getMethod("parse", new Class[] {String.class});

      Method getDocument = c.getMethod("getDocument", null);
    ...
    if (doc != null) {
      ...
      if (exceptions != null) {
        StringBuffer b =
            new StringBuffer(
                "I was Unable to locate appropriate XML parser "
        ...
        msg =
            "An exception occurred while instantiating the XML document.  "
  ...
  public static String getText(Element e) {
    ...
    if (len == 0)
      throw new IllegalArgumentException("The element " + e + " does not have any child nodes.");

    for (int i = 0; i < len; i++) {
      Node node = nodes.item(i);
      if (node.getNodeType() == Node.TEXT_NODE) return ((org.w3c.dom.Text) node).getData();
    }

    throw new IllegalArgumentException("The element " + e + " does not have any text nodes.");
  }
