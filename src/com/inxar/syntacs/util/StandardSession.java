...
public class StandardSession implements Session {
  ...
  public StandardSession(Writer out) {
    ...
    if (out == null) this.out = new BufferedWriter(new OutputStreamWriter(System.out));
    else if (!(out instanceof BufferedWriter)) this.out = new BufferedWriter(out);
    else this.out = out;
  ...
  public synchronized Log log(String name, Object obj) {
    ...
    buf.add('[').add(name).add(']');

    regbuf.add('"').add(name).add('"').add(" was ").add(obj.getClass().getName()).ln();
  ...
  private synchronized void write(StandardChannel channel, boolean writeln) {
    if (meter != null && meter.isActive()) unmeter();
    ...
    try {
      ...
      synchronized (out) {
        ...
        if (writeln) out.write(StringTools.NEWLINE);
  ...
  private void body(StandardChannel channel) throws IOException {
    ...
    for (int i = 0; i < len; i++) {
      ...
      switch (src[i]) {
        ...
        case '\n':
          ...
          if (off != i) out.write(src, off, i - off);
    ...
    if (off != len) out.write(src, off, len - off);
  }
  ...
  private void writeTime1(StandardChannel channel) {
    ...
    write(channel.buf.add(" [").add(time).add(" ms]").ln());
  ...
  private void writeTime2(StandardChannel channel) {
    ...
    Buffer b = channel.buf.add("Done [").add(time).add(" ms]");
  ...
  public synchronized void close() {
    if (out != null) {
      ...
      try {
        ...
        write(new Buffer().add("---------------------------------------------").ln());

        write(regbuf);

        write(new Buffer(45).add("Session ended at ").add(new java.util.Date().toString()).ln());
  ...
  public void put(String key, Object val) {
    if (DEBUG) {
      ...
      if (val instanceof String) debug.quote(val);
      else debug.quote(val.getClass().toString());
  ...
  public boolean getBoolean(String key) {
    ...
    else throw new NullPointerException("No boolean value under " + key);
  }
  ...
  public int getInt(String key) {
    ...
    else throw new NullPointerException("No int value under " + key);
  }
  ...
  private Log log() {
    if (log == null) log = Mission.control().log("ssn", this);
    return log;
  }
  ...
  private class StandardLog implements Log {
    StandardLog(Buffer name) {
      this.name = name;
    }

    public Channel trace() {
      if (trace == null) trace = instantiate("[trce]: ");
      return trace;
    }

    public Channel debug() {
      if (debug == null) debug = instantiate("[dbug]: ");
      return debug;
    }

    public Channel info() {
      if (info == null) info = instantiate("[info]: ");
      return info;
    }

    public Channel warn() {
      if (warn == null) warn = instantiate("[warn]: ");
      return warn;
    }

    public Channel critical() {
      if (critical == null) critical = instantiate("[crit]: ");
      return critical;
    }

    public Channel catastrophe() {
      if (catastrophe == null) catastrophe = instantiate("[cata]: ");
      return catastrophe;
    }

    private Channel instantiate(String level) {
      return StandardSession.this.new StandardChannel(name, new Buffer().add(level));
    }
  ...
  private class StandardChannel implements Channel {
    ...
    public Channel back() {
      if (tab.length() >= 1) tab.length(tab.length() - 1);
      return this;
    }
