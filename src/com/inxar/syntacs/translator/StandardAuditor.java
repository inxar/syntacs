 ...
 */
public class StandardAuditor implements Auditor, Serializable {
  ...
  public Complaint notify(int type, String msg, Input in, int off, int len) {
    return notify(this.new StandardComplaint(type, in.atln(), msg, trace(in, off, len)));
  }
  ...
  public Complaint notify(Complaint d) {
    switch (d.getType()) {
      ...
      case Complaint.UNSPECIFIED_ERROR:
        ...
        break;
      default:
        throw new IllegalArgumentException("Unknown Complaint Type: " + d.getType());
    }
    return d;
  }
  ...
  protected String trace(Input in, int off, int len) {
    if (DEBUG)
      log().debug().write("Trace request at off ").write(off).write(" and len ").write(len).out();
    ...
    StringBuffer head = new StringBuffer().append("  \\   Line ").append(in.atln()).append(": ");
    ...
    if (DEBUG) log().debug().write("line is ").quote(in.stretch(beg, end - beg)).out();
    ...
    if (len == 0) b.append('*');
    else if (len == 1) b.append('^');
    else {
      b.append('<');

      while (len-- > 2) b.append('-');
  ...
  public String toString() {
    ...
    if (errors.size() > 0) {
      b.append(errors.size()).append(errors.size() == 1 ? " Error" : " Errors");

      if (src != null) b.append(" in ").append(src).append(':');
      else b.append(':');
    ...
    if (warnings.size() > 0) {
      b.append(warnings.size()).append(warnings.size() == 1 ? " Warning" : " Warnings");
  ...
  private Log log() {
    if (log == null) log = Mission.control().log("err", this);
    return log;
  }
  ...
  public class StandardComplaint implements Complaint, Serializable {
    ...
    private void init() {
      ...
      switch (type) {
        case Complaint.SEMANTIC_WARNING:
          b.append("*-- Semantic Warning");
          break;
        case Complaint.LEXICAL_ERROR:
          b.append("*-- Lexical Error");
          break;
        case Complaint.SYNTACTIC_ERROR:
          b.append("*-- Syntactic Error");
          break;
        case Complaint.SEMANTIC_ERROR:
          b.append("*-- Semantic Error");
          break;
        case Complaint.UNSPECIFIED_ERROR:
          b.append("*-- Error");
          break;
      ...
      if (trace != null)
        b.append(StringTools.NEWLINE).append(" \\").append(StringTools.NEWLINE).append(trace);
      else b.append(StringTools.NEWLINE).append(" \\___ ");
