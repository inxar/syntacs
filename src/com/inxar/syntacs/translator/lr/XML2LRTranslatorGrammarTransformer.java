...
public class XML2LRTranslatorGrammarTransformer
{
    ...
    private synchronized TranslatorGrammar _transform(String uri, Element doc) throws AlgorithmException
    {
	this.doc = doc;
	...
	while (i.hasNext()) {
	    ...
	    String name = DOM.getString("name", e);
	    g.addContext(name);
	    if (DOM.getBoolean("start", e))
		g.setStartContext(name);
	}
    }
    ...
    {
	if (log == null)
	    log = Mission.control().log("x2g", this); // Xml To lrtranslatorGrammar
	return log;
    }
	...
	try {
	    ...
	    switch (2) {
	    case 1:
		g = transformXML(uri); break;
	    case 2:
		g = new SyntacsGrammar(); break;
	    default:
