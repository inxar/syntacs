 ...
 */
public class StandardLRTranslatorGrammar implements LRTranslatorGrammar {
  ...
  */
  ...
  public Translator newTranslator(Properties p) {
    if (translator == null) {
      ...
      if (DEBUG) log().debug().write("Compiling translator...").out();
      ...
      if (DEBUG) log().debug().write("Done compiling lexer.").out();
  ...
  public int getContextAction(int contextID, int symbolID) {
    ...
    if (s == null) return UNDEFINED_CONTEXT;

    if (!s.hasAction(symbolID)) return UNDEFINED_SYMBOL;
  ...
  public boolean isTerminal(int ID) {
    ...
    } else throw new IllegalArgumentException("Undefined Terminal");
  }
  ...
  */
  ...
  public void addContext(String name) {
    if (ALL.equals(name)) {
      ...
      if (all != null)
        addSemanticError("Context `" + name + "' was declared twice (duplicate declaration).");
      else {
  ...
  public void setStartContext(String name) {
    if (ALL.equals(name)) {
      addSemanticError("`all' is a virtual context -- it cannot be the start context.");
    } else if (hasStartContext()) {
      addSemanticError(
          "Can't set the start context to `"
              + name
              + "' since it was previously defined as `"
              + getContext(getStartContext())
              + "' (duplicate definition)");
    } else {

      int ID = getContext(name);
      if (ID == UNDEFINED_CONTEXT)
        addSemanticError("Can't set start context to undefined context `" + name + "'");
      else {
  ...
  public void setContextPush(String contextName, String symbolName, String contextPushed) {
    ...
    if (ALL.equals(contextPushed)) {
      addSemanticError(
          "Illegal context stack instruction for context `"
              + contextName
              + "', symbol `"
              + symbolName
    ...
    if (s.hasAction(symbolID)) {
      ...
      addSemanticError(
          "Illegal context stack instruction for context `"
              ...
              + "', symbol `"
              + symbolName
              + "':  Duplicate definition (already "
              + getActionName(s, symbolID)
              + ")");
  ...
  private String getActionName(ContextSymbol s, int symbolID) {
    ...
    switch (action) {
      ...
      case ACTION_POP:
        return "POP";
      case ACTION_PUSH:
        return "PUSH " + getContext(s.getRegister(symbolID));
      default:
  ...
  public void addTerminal(String name) {
    if (hasTerminal(name)) addSemanticError("Duplicate definition of terminal `" + name + "'.");
    else {
  ...
  public void setTerminalRegexp(String name, Object regexp) {
    if (!hasTerminal(name)) {
      ...
      if (s.regexp != null)
        addSemanticError("Duplicate match definition for terminal `" + name + "'.");
      else {
        s.regexp = regexp;
        setChanged();
      }
      ...
      if (s.hasContext(contextID)) {
        throw new InternalError(
            "Terminal "
                ...
                + " is already a member of context "
  ...
  public void addNonTerminal(String name) {
    if (hasNonTerminal(name))
      addSemanticError("Duplicate definition of nonterminal `" + name + "'.");
    else {
      int ID = symbols.length() + 1;
  ...
  public void setGoalNonTerminal(String name) {
    if (hasGoalNonTerminal()) {
      ...
      if (ID == UNDEFINED_SYMBOL || ID == UNDEFINED_NONTERMINAL)
        addSemanticError("Illegal goal symbol definition: undefined nonterminal `" + name + "'");
      else {
        this.goal = ID;
        setChanged();
      }
  ...
  public void addProduction(String nonterminalName, List symbolNames) {
    ...
    for (int i = 0; i < symbolNames.size(); i++) {
      ...
      if (ID == UNDEFINED_SYMBOL) undefs.add(i);
      else {
        if (DEBUG)
          log()
              .debug()
    ...
    if (undefs.length() != 0) {
      b =
          new StringBuffer("There was a problem while trying to add a ")
              .append("production for the rule `")
              .append(name)
              .append("' .  The problem is that ");
      if (undefs.length() == 1) {
        b.append("the symbol `").append(symbolNames.get(undefs.at(0))).append("' is undefined.)");
      } else {
      ...
      if (hasProduction(name)) {
        addSemanticError("Duplicate declaration of production `" + name + "'.");
      } else {

        int nonterminalID = getNonTerminal(nonterminalName);
        if (nonterminalID == UNDEFINED_NONTERMINAL || nonterminalID == UNDEFINED_SYMBOL)
          addSemanticError(
              "Illegal production definition: nonterminal `" + nonterminalName + "' is undefined.");
        else {
          productions.add(new ProductionSymbol(productions.length(), name, nonterminalID, syms));
          setChanged();
        }
      }
    }
  ...
  public void compile(Properties p) {
    if (hasChanged()) {
      ...
      compileRegularSets(p);
      errcheck();
      compileDFAs(p);
      errcheck();
      compileLexer(p);
      errcheck();
      compileContextFreeSet(p);
      errcheck();
      compileLRTranslatorGrammar(p);
      errcheck();
      compileDPA(p);
      errcheck();
      compileParser(p);
      errcheck();
      compileInterpreter(p);
      errcheck();
      compileLRTranslator(p);
      errcheck();
  ...
  private void compileContexts() {
    ...
    if (contexts.length() > 1) {
      ...
      if (all != null) {
        ...
        if (t.length() == 0) {
          addSemanticWarning(
              "Vacuous definition of context `all' " + "(should contain at least one terminal).");
        } else {
      ...
      if (!isDefaultExplicit) {
        ...
        for (int i = 0; i < a.length(); i++) setContextPeek(DEFAULT, getTerminal(a.at(i)));
      }
    }
    ...
    for (int i = 0; i < c.length(); i++) {
      ...
      if (t.length() == 0) {
        String cName = getContext(cID);
        addSemanticError("Context `" + cName + "' has no terminals.  A context must not be empty.");
      }
    }
    ...
    if (!hasStartContext()) setStartContext(DEFAULT);
  }
  ...
  private void compileNonTerminals() {
    if (nonterminals.length() == 0)
      addSemanticError("There were no defined nonterminals.  At least one is required.");
  }

  private void compileProductions() {
    if (productions.isEmpty())
      addSemanticError("There were no defined productions.  At least one is required.");
    if (!hasGoalNonTerminal())
      addSemanticError("A goal symbol (nonterminal) was not defined, but one needs to be.");
  }
  ...
  private void compileNames(Properties p) {
    ...
    for (int i = 0; i < len; i++) names[i] = name + StringTools.capitalize(getContext(i)) + "DFA";
  ...
  private void compileRegularSets(Properties p) {
    String[] names = null;
    if (verbose) names = (String[]) Mission.control().get("_dfa-names");
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (verbose) log().info().write("Building regular grammar for ").write(names[i]).time();
  ...
  private void compileDFAs(Properties p) {
    ...
    for (int i = 0; i < len; i++) {
      if (verbose) log().info().write("Building ").write(names[i]).time();

      t_dfas[i] = (TreeDFA) new TreeDFAConstructor().construct(regularSets[i]);

      if (Mission.control().isTrue("compile-grammar-regular-debug"))
        log().debug().write(regularSets[i]).out();

      if (verbose) log().info().write("Compressing ").write(names[i]).time();

      if (Mission.control().isTrue("compile-dfa-debug")) log().debug().write(t_dfas[i]).out();

      m_dfas[i] = (MesoArrayDFA) new Tree2MesoArrayDFATransformer().transform(t_dfas[i]);
  ...
  private void compileLexer(Properties p) {
    ...
    Lexer lexer =
        (Lexer)
            reflect("compile-lexer-classname", "com.inxar.syntacs.analyzer.lexical.StandardLexer");
  ...
  private void compileContextFreeSet(Properties p) {
    if (verbose) {
      ...
      log().info().write("Building context-free grammar for ").write(name).time();
    }
    ...
    for (int i = 0; i < r.length(); i++) {
      ...
      if (DEBUG) log().debug().write("Adding production `").write(pName).write("' to CFG").out();
      ...
      if (s == null || s.length() == 0)
        throw new IllegalArgumentException(
            "Production `" + getProduction(pID) + "' has empty list for RHS.");
      ...
      for (int j = 0; j < s.length(); j++) {
        ...
        if (symbol == null)
          throw new NullPointerException("Local map lookup of symbol `" + symName + "' was null");

        rule.add(symbol);
      }

      // Set the start production if it reduces to goal.
      if (getGoalNonTerminal() == ntID) g.setStartProduction(rule);
    }
  ...
  private void compileDPA(Properties p) {
    ...
    try {
      ...
      if (verbose) {
        ...
        log().info().write("Building ").write(name).time();
      }

      String ctorType = Mission.control().getString("compile-dpa-constructor-method", "lalr1");

      if (ctorType != null) {
        ctorType = ctorType.toLowerCase();
        if ("lalr1".equals(ctorType)) ctor = new LALR1Constructor();
        ...
        else if ("lr1".equals(ctorType)) ctor = new LR1Constructor();
        else {
          auditor.notify(
              Complaint.UNSPECIFIED_ERROR,
              "Illegal dpa constructor method: `"
                  + ctorType
      ...
      if (verbose) log().info().write("Compressing ").write(name).time();

      ma_dpa = (MesoArrayDPA) new Array2MesoArrayDPATransformer().transform(a_dpa);

      if (Mission.control().isTrue("compile-dpa-debug")) log().debug().write(ctor).out();
  ...
  private void compileParser(Properties p) {
    ...
    Parser parser =
        (Parser)
            reflect(
                "compile-parser-classname", "com.inxar.syntacs.analyzer.syntactic.StandardParser");
  ...
  private void compileInput(Properties p) {
    Input input =
        (Input) reflect("compile-input-classname", "com.inxar.syntacs.analyzer.StandardInput");
  ...
  private Object reflect(String key, String def) {
    ...
    try {
      ...
      if (className != def) {
        ...
        try {
          o = Class.forName(def).newInstance();
        } catch (Exception ex2) {
        }
      } else throw new InternalError("Bad property name management.");
    }
  ...
  private void errcheck() {
    if (auditor.hasErrors())
      throw new RuntimeTranslationException(auditor, "Translator construction failed.");
  }
  ...
  public String toString2() {
    if (version == null) setVersion("0.0.1");
    ...
    for (int i = 0; i < a.length(); i++) {
      ...
      if (startContext == cID) b.append(" <start>");
      ...
      if (t == null || t.length() == 0) {
        ...
        for (int j = 0; j < t.length(); j++) {
          int tID = t.at(j);
          b.append("   -- ").append(getTerminal(tID)).append(" -> ");
          int action = getContextAction(cID, tID);
          switch (action) {
            case ACTION_PEEK:
              b.append("PEEK");
  ...
  public String toSyntacs() {
    ...
    for (int i = 0; i < a.length(); i++) {
      int pID = a.at(i);
      b.append("reduce ").append(getNonTerminal(getProductionNonTerminal(pID))).append(" when");
  ...
  private void writeFile(String dir, String file, String ext, String text) {
    ...
    try {
      ...
      if (dir != null && dir.length() > 0) {
        filename.append(dir);
        if (dir.charAt(dir.length() - 1) != File.separatorChar) filename.append(File.separatorChar);
      }
  ...
  private Log log() {
    if (this.log == null) this.log = Mission.control().log("lrg", this); // LRtranslatorGrammar
    return log;
  }
  ...
  */
  ...
  private class ContextSymbol extends Symbol {
    ...
    boolean hasRegister(int ID) {
      if (ID >= registers.size()) return false;
      else return registers.get(ID) != 0;
    }
    ...
    private void addSymbol(int symbolID) {
      if (!symbols.contains(symbolID)) symbols.add(symbolID);
    }
  ...
  private static class SymbolList implements IntArray {
    ...
    public int[] toArray() {
      ...
      for (int i = 0; i < count; i++) dst[i] = src[i].ID;
      return dst;
    }

    void add(Symbol s) {
      while (count >= src.length) enlarge();
      src[count++] = s;
    }
    ...
    int getID(String name) {
      for (int i = 0; i < count; i++) if (src[i].name.equals(name)) return src[i].ID;
      return error;
    }
    ...
    Symbol getSymbol(int ID) {
      ...
      for (int i = Math.min(count - 1, ID); i >= 0; i--) if (src[i].ID == ID) return src[i];

      return null;
    }

    Symbol getSymbol(String name) {
      for (int i = 0; i < count; i++) if (src[i].name.equals(name)) return src[i];
      return null;
    }

    boolean contains(int ID) {
      for (int i = 0; i < count; i++) if (src[i].ID == ID) return true;
      return false;
    }

    boolean contains(String name) {
      for (int i = 0; i < count; i++) if (src[i].name.equals(name)) return true;
      return false;
    }
