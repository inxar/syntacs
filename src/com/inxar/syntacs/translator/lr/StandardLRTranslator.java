...
public class StandardLRTranslator implements LRTranslator {
  ...
  public Object translate(Object src) throws TranslationException {
    if (DEBUG) log().debug().write("Translating ").write(src).time();
    ...
    try {
      ...
      // Check
      if (grammar == null) throw new NullPointerException("Undefined LRTranslatorGrammar");

      if (in == null) throw new NullPointerException("Undefined Input");

      if (lexer == null) throw new NullPointerException("Undefined Lexer");

      if (inter == null) throw new NullPointerException("Undefined Interpreter");

      if (parser == null) throw new NullPointerException("Undefined Parser");
      ...
      if (!auditor.isEmpty()) throw new TranslationException(auditor);
  ...
  private Log log() {
    if (log == null) log = Mission.control().log("lrt", this); // LRTranslator
    return log;
  }
