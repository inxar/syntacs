...
public class LRTranslatorGrammarBurner implements Burner {
  ...
  public void burn(Object src, ClassDeclaration cls) {
    if (!(src instanceof LRTranslatorGrammar))
      throw new IllegalArgumentException(
          "I can only generate instances of "
              + LRTranslatorGrammar.class
              + ", not "
    ...
    if (c != null) {
      s = c.getText() + "<P>" + s;
      c.setText(s);
    } else cls.setComment(Comment.D, s);
  }
  ...
  private void newMethod_getName() {
    newMethod(t_String, "getName").newReturn().setExpression(vm.newString(g.getName()));
  }

  private void newMethod_getVersion() {
    newMethod(t_String, "getVersion").newReturn().setExpression(vm.newString(g.getVersion()));
  }

  private void newMethod_getStartContext() {
    newMethod(t_int, "getStartContext").newReturn().setExpression(vm.newInt(g.getStartContext()));
  }

  private void newMethod_getGoalNonTerminal() {
    newMethod(t_int, "getGoalNonTerminal")
        .newReturn()
        .setExpression(vm.newInt(g.getGoalNonTerminal()));
  }
  ...
  private void newMethod_getTerminalRegexp() {
    ...
    for (int i = 0; i < keys.length(); i++) {
      ...
      if (o != null) vals[i] = vm.newString(o.toString());
      else vals[i] = l_null;
    }
  ...
  private void newMethod_getProductionLength() {
    IntArray keys = g.getProductions();
    Expression[] vals = new Expression[keys.length()];
    for (int i = 0; i < keys.length(); i++) vals[i] = vm.newInt(g.getProductionLength(keys.at(i)));
  ...
  private void newMethod_getProductionSymbols() {
    ...
    for (int i = 0; i < keys.length(); i++) {
      ...
      if (syms != null) {
        ...
        for (int j = 0; j < syms.length(); j++) vals[i][j] = syms.at(j);

        e[i] = newIntArray(vm.newArrayAccess(null, "productionSymbols").addDim(vm.newInt(i)));
      } else {
        vals[i] = null;
        e[i] = l_null;
      }
  ...
  private void newMethod_getContextTerminals() {
    ...
    for (int i = 0; i < keys.length(); i++) {
      ...
      if (syms != null) {
        ...
        e[i] = newIntArray(vm.newArrayAccess(null, "contextTerminals").addDim(vm.newInt(i)));
      } else {
        vals[i] = null;
        e[i] = l_null;
      }
  ...
  private void newMethod_getTerminalContexts() {
    ...
    for (int i = 0; i < keys.length(); i++) {
      ...
      if (syms != null) {
        ...
        e[i] = newIntArray(vm.newArrayAccess(null, "terminalContexts").addDim(vm.newInt(i)));
      } else {
        vals[i] = null;
        e[i] = l_null;
      }
  ...
  private void newMethod_getContextActionAndRegister() {
    ...
    newMethod_switch(t_int, "getContextAction", "cID", c, eA, l_UNC).addParameter(t_int, "tID");

    newMethod_switch(t_int, "getContextRegister", "cID", c, eR, l_UNC).addParameter(t_int, "tID");
  }
  ...
  private void newMethod_newTranslator2() {
    ...
    else e_interpreter = l_null;

    NewClass nc =
        vm.newClass(t_StandardTranslator)
  ...
  private void newMethod_newTranslator3() {
    ...
    set(m, "LRTranslatorInterpreter", "_interpreter", "interp", "setLRTranslatorInterpreter");

    // Initialize the lexer
    String[] dfaNames = (String[]) Mission.control().get("_dfa-names");
    Expression[] ae = new Expression[dfaNames.length];
    for (int i = 0; i < ae.length; i++) ae[i] = vm.newClass(vm.newType(dfaNames[i]));
    ...
    m.newStmt(vm.newInvoke("parser", "initialize").addArg(vm.newClass(vm.newType(dpaName))));
  ...
  private void set(
      ClassMethod m, String className, String implKey, String varName, String methodName) {
  ...
  private Expression makeDFAArray(String[] classnames) {
    ...
    for (int i = 0; i < ae.length; i++) ae[i] = vm.newClass(vm.newType(classnames[i]));
  ...
  private ClassMethod newMethod_switch(
      Type type, String mName, String pName, IntArray keys, Expression[] vals, Expression def) {
    ClassMethod m = newMethod(type, mName);
    m.addParameter(t_int, pName);

    Switch s = m.newSwitch(vm.newVar(pName));
    for (int i = 0; i < keys.length(); i++)
      s.newCase(vm.newInt(keys.at(i))).newReturn().setExpression(vals[i]);
  ...
  private void newMethod_switch_inverse(String mName, String pName, IntRelation f, Expression def) {
    ...
    for (Reiterator i = f.reiterator(); i.hasNext(); i.next()) {
      ...
      if (values.length() > 0) {
        Case last = null;
        for (int j = 0; j < values.length(); j++) last = s.newCase(vm.newInt(values.at(j)));
  ...
  private Log log() {
    if (log == null) log = Mission.control().log("lrtg-burner", this);
    return log;
  }
