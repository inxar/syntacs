...
public class VizController {
  ...
  public void viz() {
    if (Mission.control().isFalse("viz")) return;
  ...
  private void init() {
    ...
    if (srcpath == null) srcpath = Mission.control().getString("compile-sourcepath", ".");

    String namespace = Mission.control().getString("viz-namespace");

    if (namespace == null) namespace = Mission.control().getString("compile-namespace");
  ...
  private void vizLexical() {
    if (Mission.control().isNotTrue("viz-lexical")) return;

    String[] names = (String[]) Mission.control().get("_dfa-names");

    TreeDFA[] dfas = (TreeDFA[]) Mission.control().get("_tree-dfas");
  ...
  private void vizSyntactic() {
    if (Mission.control().isNotTrue("viz-syntactic")) return;
    ...
    if (ctor instanceof Vizualizable) {
      ...
      if (mthd != null) name += "." + mthd.toLowerCase();
  ...
  private void emit(String name, GraphViz dot) {
    ...
    try {
      ...
      if (verbose) log().debug().write("Writing ").write(filename).write("...").time();
  ...
  private Log log() {
    if (log == null) log = Mission.control().log("vct", this); // Vizualization ConTroller
    return log;
  }
