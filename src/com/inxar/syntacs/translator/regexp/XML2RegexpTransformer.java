...
public class XML2RegexpTransformer {
  ...
  public Regexp transform(Element e) {
    ...
    if ("char".equals(name)) r = new RegexpAtom(DOM.getChar("value", e));
    else if ("epsilon".equals(name)) r = new RegexpEpsilon();
    else if ("interval".equals(name))
      r = new RegexpRange(DOM.getChar("lo", e), DOM.getChar("hi", e));
    else if ("string".equals(name)) r = Regexp.toConcat(DOM.getString("value", e));
    ...
    else if ("class".equals(name)) r = transformCharClass(e);
    else if ("list".equals(name)) r = transformList(Regexp.CONCAT, e);
    else if ("union".equals(name)) r = transformList(Regexp.UNION, e);
    else throw new IllegalArgumentException("Unrecognized tag having name \"" + name + "\"");

    if (DEBUG) log().debug().write("in parse(Element): built ").write(r).out();
  ...
  protected Regexp transformCharClass(Element e) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if ("char".equals(name)) {
        ...
        if (c == '-') cc.hasDash(true);
        else cc.addRegexp(new RegexpAtom(c));
      } else {
  ...
  private Log log() {
    if (log == null) log = Mission.control().log("x2r", this);
    return log;
  }
