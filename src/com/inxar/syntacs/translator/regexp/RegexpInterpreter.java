...
public class RegexpInterpreter extends StandardLRTranslatorInterpreter {
  ...
  public void match(int type, int off, int len) throws TranslationException {
    if (DEBUG) log().debug().write("seeing input ").write(type).out();
    ...
    switch (type) {
      ...
      case RegexpGrammar.T_CHAR_CLASS_CHAR:
        if (len != 1) throw new InternalError("Expected token to be only one character.");
      ...
      case RegexpGrammar.T_ESC:
        ...
        switch (c) {
          ...
          default:
            auditor.notify(Complaint.LEXICAL_ERROR, "Undefined literal escape", in, off, len);
        }
        symbol = newAtom(type, c);
        break;

      case RegexpGrammar.T_OPEN_BRACKET:
        symbol = newClass(type, false, false);
        break;
      case RegexpGrammar.T_OPEN_BRACKET_CARET:
        symbol = newClass(type, true, false);
        break;
      case RegexpGrammar.T_OPEN_BRACKET_DASH:
        symbol = newClass(type, false, true);
        break;
      case RegexpGrammar.T_OPEN_BRACKET_CARET_DASH:
        symbol = newClass(type, true, true);
        break;

      case RegexpGrammar.T_STAR:
        symbol = new QuantifierSymbol(type, Regexp.CLOSURE);
        break;
      case RegexpGrammar.T_QUESTION:
        symbol = new QuantifierSymbol(type, Regexp.OPTIONAL);
        break;
      case RegexpGrammar.T_PLUS:
        symbol = new QuantifierSymbol(type, Regexp.PCLOSURE);
        break;

      default:
        throw new InternalError("Expected terminal type: " + grammar.getTerminal(type));
    }
  ...
  private Log log() {
    if (log == null) log = Mission.control().log("regi", this);
    return log;
  }
  ...
  private static class QuantifierSymbol implements Symbol {
    ...
    public String toString() {
      switch (regexpType) {
        case Regexp.OPTIONAL:
          return "?";
        case Regexp.CLOSURE:
          return "*";
        case Regexp.PCLOSURE:
          return "+";
        default:
  ...
  private static class CharClassBeginSymbol implements Symbol {
