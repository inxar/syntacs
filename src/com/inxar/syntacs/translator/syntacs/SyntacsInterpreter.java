...
public class SyntacsInterpreter extends StandardLRTranslatorInterpreter
{
	...
	case SyntacsGrammar.T_IDENT:
	    ...
	    if (ID != null)
		symbol = new ConstantSymbol(ID.intValue());
	    else
		symbol = new StringSymbol(type, str);
	    break;
	...
	if (symbol == null)
	    throw new InternalError("Null terminal for "+grammar.getTerminal(type));
	...
	case SyntacsGrammar.P_NonterminalStatement__NONTERMINAL_IdentList_SEMI:
	    iter = ((ListSymbol)s.at(1)).list.iterator();
	    while (iter.hasNext())
		g.addNonTerminal( (String)iter.next() );
	...
	default:
	    throw new InternalError
		("Unknown Production: "+grammar.getProduction(type));
	}
    ...
    {
	if (VERBOSE)
	    log().info().write("Parsing stt...").time();
    }
    ...
    {
	if (log == null)
	    log = Mission.control().log("syn", this);
	return log;
    }
